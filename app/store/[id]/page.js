        ...
        try {
            ...
            const options = {
                ...
                modal: {
                    ...
                    ondismiss: function () {
                        setLoading(false)
                        // Remove body class and styles when modal closes
                        document.body.classList.remove('razorpay-open')
                        document.body.style.overflow = ''
                        document.body.style.position = ''
                        document.body.style.width = ''
            ...
            // Add body class and styles when modal opens
            ...
            // Force z-index and positioning after a short delay
            setTimeout(() => {
                ...
                const razorpayIframe = document.querySelector('iframe[name^="razorpay"]')

                if (razorpayContainer) {
                    razorpayContainer.style.zIndex = '2147483647'
                    razorpayContainer.style.position = 'fixed'
                }
                if (razorpayBackdrop) {
                    razorpayBackdrop.style.zIndex = '2147483646'
                    razorpayBackdrop.style.position = 'fixed'
                    razorpayBackdrop.style.top = '0'
                    razorpayBackdrop.style.left = '0'
                    razorpayBackdrop.style.right = '0'
                    razorpayBackdrop.style.bottom = '0'
                    razorpayBackdrop.style.width = '100vw'
                    razorpayBackdrop.style.height = '100vh'
                }
                if (razorpayFrame) {
                    razorpayFrame.style.zIndex = '2147483647'
                    razorpayFrame.style.position = 'fixed'
                }
                if (razorpayIframe) {
                    razorpayIframe.style.zIndex = '2147483647'
                    razorpayIframe.style.position = 'fixed'
                }
            }, 100)

            // Additional check after 500ms for slower devices
            setTimeout(() => {
                const razorpayContainer = document.querySelector('.razorpay-container')
                if (razorpayContainer) {
                    razorpayContainer.style.zIndex = '2147483647'
